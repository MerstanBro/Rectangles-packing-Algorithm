...
/**
 * Yield to the event loop (and optionally pause) so the UI can paint.
 * Uses setTimeout so callers can drive solving with interval-like pacing.
 */
function wait(ms = 0) {
...
function assertNotAborted(signal) {
  if (signal?.aborted) {
    const err = new Error("Solving aborted");
    err.name = "AbortError";
    throw err;
  }
}

async function TryAllPossibilites(
  possibilites,
  Rectagnles,
  widthScale,
  lengthScale,
  ...
  const { onSolution, intervalMs = 0, signal } = options;
  let ans = [];
  for (const pos of possibilites) {
    ...
    if (answer === false) {
      ...
      ans.push(solution);
      if (typeof onSolution === "function") {
        await onSolution(solution, ans);
      }
      if (intervalMs > 0) {
        await wait(intervalMs);
      } else {
        // Always yield once so React can render incremental results
        await wait(0);
      }
...
/**
 * Enumerate subsets of rectangles and try packing each one.
 * Supports async pacing via options.intervalMs and live updates via options.onProgress.
 */
async function TakeItOrLeaveIt(
  Rectangles,
  TakenRectangles = [],
  str = '',
  visited = [],
  ...
  const { onProgress, intervalMs = 0, signal } = options;
  assertNotAborted(signal);
  let numberOfAns = 0
  ...
  if (TakenR.length !== 0 && visited[Str] === undefined && Str !== '') {
    ...
    visited[Str] = true;
    let lalo = await TryAllPossibilites(
      ...
      details.lengthScale,
      {
        intervalMs,
        signal,
        onSolution: async (solution) => {
          Answers.push(solution.Rectangles);
          if (typeof onProgress === "function") {
            await onProgress({
              latest: solution.Rectangles,
              Answers,
              numberOfAns: Answers.length,
              subset: Str,
  ...
  // Sequential take/leave — order mutates TakenR/Str; must not run in parallel
  for (const rect of Rectangles) {
    assertNotAborted(signal);
    //console.log(`take or leave ${rect.name}`)
    Recs = Recs.filter((r) => r.name !== rect.name)
    let leaveIt = await TakeItOrLeaveIt(Recs, TakenR, str, visited, Answers, options)
    ...
    TakenR.push(rect)
    Str += rect.name
    let takeIt = await TakeItOrLeaveIt(Recs, TakenR, Str, visited, Answers, options)
...
async function solve(string = new String()) {
  ...
  console.log(details);
  let acceptedFormations = await TryAllPossibilites(
    details.possibilites,
    connectecdRectangles,
    details.widthScale,
    details.lengthScale,
  ...
  let asda
  console.log('------------')
  asda = await TakeItOrLeaveIt(connectecdRectangles)
...
/**
 * Find all packable subsets of the given rectangles.
 *
 * @param {Array} Rectagnles - rectangles with { name, width, length }
 * @param {object} [options]
 * @param {number} [options.intervalMs=0] - pause between found solutions (setInterval-like pacing)
 * @param {(info: { latest, Answers, numberOfAns, subset?, width?, length? }) => void|Promise<void>} [options.onProgress]
 * @param {AbortSignal} [options.signal] - abort mid-solve
 * @returns {Promise<{ numberOfAns: number, Answers: Array }>}
 */
export async function CheckForRecs(Rectagnles, options = {}) {
  let aaa = await TakeItOrLeaveIt(Rectagnles, [], '', [], [], options)
